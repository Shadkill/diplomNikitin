...
const UpdatePageCar = () => {
    ...
    const handleUpdatePriceBackend = async(newPrice)=>{
        try {
            const response = await axios.put(`http://localhost:5000/api/updateCarPrice/${_id}`, {price:Number(newPrice)});
            if(response.status === 200){
                toast.success(response.data.message);
                setCar(prevCar =>({...prevCar,price:newPrice}))
                setIsUpdatePrice(false);
            setUpdateValue('');
            }else {
    ...
    const handleUpdateTitleBackend = async(newTitle) =>{
        try {
            const response = await axios.put(`http://localhost:5000/api/updateCarTitle/${_id}`, {title:newTitle});
            if(response.status===200){
                toast.success(response.data.message);
                setCar(prevCar=>({...prevCar, name:newTitle}));
                setIsUpdate(false);
                setUpdateValue('')
            }else {
            toast.error("Ошибка при обновлении");
        ...
        <>
            ...
            <div className="form_bid" >
                ...
                <div className="div_form_bid">
                    <div className="block_t">{isUpdate ? <><input type="text" value={updateValue} onChange={(e)=>setUpdateValue(e.target.value)} className="inp_val"/>
                     <div className="classButton"> <button className="butAprove" onClick={()=>handleUpdateTitleBackend(updateValue)}>Подтвердить</button> <button onClick={()=> handleClickReject()} className="butReject">Отмена</button> </div></>:<><h1 className="name_car_id">Марка: {car.name} </h1><p onClick={()=>handleUpdateTitle(car.name)}>Изменить</p> </>}
                    </div>
                    <div className="block_t">
                     {isUpdatePrice?<><input type="number" value={updateValue} onChange={(e)=>setUpdateValue(e.target.value)} className="inp_val"/> <div className="classButton"> <button className="butAprove" onClick={()=>handleUpdatePriceBackend(updateValue)}>Подтвердить</button> <button onClick={()=> handleClickReject()} className="butReject">Отмена</button> </div></>: <><h2 className="price_car_id">Цена: {car.price}₽/сут</h2> <p onClick={()=>handleUpdatePrice(car.price)}>Изменить</p> </>}
